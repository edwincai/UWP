    ...
    public sealed partial class MainPage : Page
    {
        ...
        private List<TodoItem> getTodoItem(string search)
        {
            ...
            using (var statement = db.Prepare("SELECT Title,Description,Date FROM TodoItem WHERE Title = ?"))
            {
                ...
                {
                    todo = new TodoItem((string)statement[0], (string)statement[1], DateTimeOffset.Parse((string)statement[2]));
                    todos.Add(todo);
                }
            }
            return todos;
        ...
        private void search_Click(object sender, RoutedEventArgs e)
        {
            ...
            for(int i = todos.Count-1; i > 0; i--)
            {
                ...
                string date = temp.day.ToString();
                var k = new MessageDialog(title + "   " + description + " " + date).ShowAsync();
            }
         }
